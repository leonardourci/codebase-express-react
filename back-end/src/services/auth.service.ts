...
import { TLoginPayload, ILoginResponse, TSignupPayload, ISignupResponse } from '../types/auth'
import { createUser, getUserByEmail } from '../database/repositories/user.repository'

const { HASH_SALT } = process.env

export async function authenticateUser(payload: TLoginPayload): Promise<ILoginResponse> {
...
export async function registerUser(payload: TSignupPayload): Promise<ISignupResponse> {
