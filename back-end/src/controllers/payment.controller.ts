...
import { PaymentRequest } from "../middlewares/payment.middleware";
...
export const processPaymentWebhookHandler = async (req: PaymentRequest, res: Response) => {
  const { paymentEvent } = req
  switch (paymentEvent.type) {
     ...
     */
    case 'invoice.paid':
      const paidInvoice = paymentEvent.data.object;
      ...
      // Exemplo: conceder acesso ao serviço

      await grantUserAccess(paidInvoice.customer);
      break;
    ...
    case 'invoice.payment_failed':
      ...
      await notifyUserOfPaymentFailure(failedInvoice.customer, failedInvoice.id);
      break;

    /**
     * EVENTO: `customer.subscription.created`
     * PORQUÊ ESCUTAR: Para detectar quando uma NOVA assinatura é criada.
     * Útil para registrar a intenção de compra, mas NÃO para conceder acesso.
     *
     * COMO/ O QUE FAZER:
     * 1. Registrar em seu sistema que uma nova assinatura foi iniciada para o cliente.
     * 2. Aguardar o evento `invoice.paid` para conceder acesso.
     * 3. Se acompanhado de um `invoice.payment_failed`, tratar como falha no primeiro pagamento.
     *
     * PONTOS DE ATENÇÃO:
     * - Uma assinatura criada pode ter status `incomplete` se o primeiro pagamento falhar.
     * - NUNCA conceda acesso baseado apenas neste evento. Aguarde o pagamento confirmado.
     */
    case 'customer.subscription.created':
      const newSubscription = event.data.object;
      // Exemplo: registrar a criação, mas não conceder acesso ainda
      await logSubscriptionCreation(newSubscription.id, newSubscription.customer);
      break;
